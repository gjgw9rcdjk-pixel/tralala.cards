...
export default function CardGame({ initialLang = 'en' }) {
  ...
  useEffect(() => {
    const s = savedLang();
    if (s) setLang(s);
    document.documentElement.lang = s || initialLang;
  }, [initialLang]);
  ...
  // Marks <body> once the player has left the intro screen, so the landing
  // page's marketing sections (rendered below #play in app/page.jsx) can
  // hide themselves via CSS while a game is in progress.
  useEffect(() => {
    if (view === 'intro') delete document.body.dataset.playing;
    else document.body.dataset.playing = 'true';
  }, [view]);

  // Deep link from the landing page's "Choose Your Vibe" cards (?vibe=<categoryId>)
  // jumps straight into a deck filtered to that category, skipping the intro screen.
  useEffect(() => {
    const vibe = new URLSearchParams(window.location.search).get('vibe');
    if (!vibe || !CATEGORIES.some((c) => c.id === vibe)) return;
    setSel([vibe]);
    setOrder(buildOrder(QUESTIONS, [vibe]));
    setPos(0);
    setView('deck');
  }, []);
        ...
        <div style={{ ...overlay, zIndex: 80, padding: 'calc(env(safe-area-inset-top) + 30px) 30px 34px', overflowY: 'auto' }}>
          <div style={{ height: '46%', flex: 'none', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
            <div style={{ position: 'relative', width: 194, height: 262, flex: 'none' }}>
              ...
              >
                <h1
                  aria-label="Tralala — Conversation Cards for Friends, Couples & Groups"
                  style={{ margin: 0, fontWeight: 400, display: 'flex', flexDirection: 'column', alignItems: 'center' }}
                >
          ...
          <div style={{ marginTop: 20, display: 'flex', flexDirection: 'column', gap: 13, flex: 1 }}>
            <p style={{ margin: 0, font: '400 12px/1.7 var(--font-mono), monospace', color: 'rgba(232,230,225,.62)' }}>{ui.introLine}</p>
          </div>
